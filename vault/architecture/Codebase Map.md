...
| `preflight.py` | Run on the Pi before a session: ports, services, undervoltage, disk, DB, then a live radar sample. Refuses READY on mock data and gives the `extendedMaxVelocity` verdict from the doppler distribution. Also runs over a recording (`--file`). See [[Nets Session Checklist]]. |
